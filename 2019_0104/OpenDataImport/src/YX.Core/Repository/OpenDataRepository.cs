...
namespace YX.Core.Repository
{
    class OpenDataRepository : IOpenDataRepository
    {
        public string ConnectionString
        {
            get
            {
                //var path = Path.Combine(Directory.GetCurrentDirectory(), @"D:\[高科大]軟體工程\github\1116\OpenDataImport\OpenDataImport\App_Data\Database.mdf");
        ...
        public List<OpenData> SelectAll(string name)
        {
            ...
            if (!string.IsNullOrEmpty(name))
                command.CommandText =
                    $"{command.CommandText} Where 資料集名稱 = N'{name}'";
        ...
        {
            var updateItem = item as YX.Core.Models.OpenData;
